...
### What is the client behavior when WebSocket connection is terminated during connection?

If the WebSocket connection is terminated when clients are in the [JoinAndReceiveIndexTask](https://github.com/aws/amazon-chime-sdk-js/blob/master/src/task/JoinAndReceiveIndexTask.ts) or [SubscribeAndReceiveSubscribeAckTask](https://github.com/aws/amazon-chime-sdk-js/blob/master/src/task/SubscribeAndReceiveSubscribeAckTask.ts), they will get `TaskFailed`, `InternalServerError` or `SignalingBadRequest` meeting session status code depending on failure reasons. If the status code is `TaskFailed` or `InternalServerError`, clients will reconnect immediately without waiting for connection timeout. If the status code is `SignalingBadRequest`, clients will not reconnect. There is a customizable [reconnectTimeout](https://github.com/aws/amazon-chime-sdk-js/blob/0147ed9fb76429a70fb0161405687af63b592fe0/src/meetingsession/MeetingSessionConfiguration.ts#L67) for builder to decide how long they want to timeout the reconnection. The default value is 2 minutes.

### What happens if the client is in mid call and their WebSocket connection is terminated?

There is a [MonitorTask](https://github.com/aws/amazon-chime-sdk-js/blob/master/src/task/MonitorTask.ts#L339) that can help monitor the WebSocket connection drop event when clients are in a mid call. The current logic won’t immediately reconnect when WebSocket connection is terminated. The monitor relies on [ReconnectionHealthPolicy](https://github.com/aws/amazon-chime-sdk-js/blob/bf282c70ca5996fa4b97fd779f29049ef3f80b0b/src/connectionhealthpolicy/ReconnectionHealthPolicy.ts#L58) to determine whether the current connection is healthy. Builders can customize the [ConnectionHealthPolicyConfiguration](https://github.com/aws/amazon-chime-sdk-js/blob/master/src/connectionhealthpolicy/ConnectionHealthPolicyConfiguration.ts) to configure the reconnect behavior. i.e. set the missed pongs upper threshold to zero to force restart the session. The current [missed pong threshold](https://github.com/aws/amazon-chime-sdk-js/blob/master/src/connectionhealthpolicy/ConnectionHealthPolicyConfiguration.ts) is 4 in JS SDK.  Reconnect will be triggered when there is 3 situations: [noPacketsReceivedRecently](https://github.com/aws/amazon-chime-sdk-js/blob/bf282c70ca5996fa4b97fd779f29049ef3f80b0b/src/connectionhealthpolicy/ReconnectionHealthPolicy.ts#L43), [missedPongsRecently](https://github.com/aws/amazon-chime-sdk-js/blob/bf282c70ca5996fa4b97fd779f29049ef3f80b0b/src/connectionhealthpolicy/ReconnectionHealthPolicy.ts#L46) and [hasBadAudioDelay](https://github.com/aws/amazon-chime-sdk-js/blob/bf282c70ca5996fa4b97fd779f29049ef3f80b0b/src/connectionhealthpolicy/ReconnectionHealthPolicy.ts#L53). When WebSocket connection is terminated, the noPacketsReceivedRecently will be firstly met and thus triggers the reconnect. The default [connectionUnhealthyThreshold](https://github.com/aws/amazon-chime-sdk-js/blob/2e2c7e36631d898b925e6e341c28f72dafcfa8d6/src/connectionhealthpolicy/ConnectionHealthPolicyConfiguration.ts#L8) is 25 which takes about less than 10 seconds to reach.
If monitor task detects the connection is terminated, it will call [connectionHealthDidChange](https://github.com/aws/amazon-chime-sdk-js/blob/2a43d4890f9d85a0b004cd32f6dc7387903d3255/src/task/MonitorTask.ts#L264) when connection health changes and reconnect will be handled from there. If the connection has been in inactive state for 2 minutes, it will no longer trigger reconnect.
